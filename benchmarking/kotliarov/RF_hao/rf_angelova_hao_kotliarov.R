...
published=read.table(paste0("N:./immunsig_scripts_manuscript/gene_lists_papers/","published_sign.txt"),sep = "\t",
...
db_genes=data.frame(Cell_type=cell_types,Gene=angelova$gene_symbol,source="Angelova_2015")

unique(db_genes$Cell_type)
# Select relevant immune cell type signatures
db_genes_filter= db_genes %>%
  subset(Cell_type %in%unique(db_genes$Cell_type)[c(1,15,11,7,16,17,22)] )

genes=db_genes_filter$Gene
length(genes)
length(unique(genes)) #218
...
saveRDS(hao_mean,paste0(dir_analysis,"hao_angelovagenes.RDS"))
hao_mean=readRDS(paste0(dir_analysis,"hao_angelovagenes.RDS"))
...
saveRDS(rf,paste0(dir_analysis,"rf_angelovagenes.RDS"))
rf=readRDS(paste0(dir_analysis,"rf_angelovagenes.RDS"))
table(rf$predicted)
...
# 1     96.45 96.48 99.19       98.97    98.56    96.46
...
# Sensitivity   PPV   NPV Specificity Accuracy f1_score
# 1       96.52 96.55 99.19       98.97    98.57    96.53
### pred 
...
# Sensitivity   PPV   NPV Specificity Accuracy f1_score
# 1     84.06 84.74 93.86       93.57    91.64     84.4
