...
RF_signatures=function(geneset, #name of the gene set, chr
  ...
  hao_mean=cbind(hao_mean,data.frame(cell_type=reference$harmonized_celltype))
  ...
  saveRDS(rf,paste0("./benchmarking/models/",file_extension,".RDS"))
  ...
  write.csv(pred,file = 
              paste0("./benchmarking/prediction_results/RF/",file_extension,
                     ".csv")
  )
